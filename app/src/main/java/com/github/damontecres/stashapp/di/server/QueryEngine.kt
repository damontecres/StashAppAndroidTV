...
class QueryEngine(
    ...
    private suspend fun <D : Operation.Data> executeQuery(query: ApolloCall<D>): ApolloResponse<D> =
        withContext(Dispatchers.IO) {
            ...
            val id = getRequestId()
            Timber.v("executeQuery $id $queryName")

            val response = query.execute()
            if (response.data != null) {
                Timber.v("executeQuery $id $queryName successful")
                ...
                Timber.e("Errors in $id $queryName: ${response.errors}")
    ...
    fun updateFilter(
        ...
        if (filter != null) {
            if (useRandom && filter.sort.getOrNull()?.startsWith("random") == true) {
                Timber.v("Updating random filter")
