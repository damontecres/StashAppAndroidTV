...
class CompanionPluginService(
    ...
    suspend fun sendLogCat(verbose: Boolean) =
        withContext(Dispatchers.IO + StashCoroutineExceptionHandler(autoToast = true)) {
            try {
                ...
                val result = sendLogMessage(sb.toString(), false)
    ...
    suspend fun sendLogMessage(
        ...
        withContext(Dispatchers.IO + StashCoroutineExceptionHandler()) {
            ...
            if (currentServer.serverPreferences.companionPluginInstalled) {
